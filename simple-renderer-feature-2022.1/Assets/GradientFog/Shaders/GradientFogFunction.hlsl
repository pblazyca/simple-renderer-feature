...
#define GRADIENTFOGFUNCTION_INCLUDED
      ...
      void FogGradient_float(float4 a, float4 b, float4 c, float t, float transparentFactor, out float4 Out) {
          ...
          Out = float4(color.r, color.g, color.b, saturate(alpha) * transparentFactor);
      }
#endif